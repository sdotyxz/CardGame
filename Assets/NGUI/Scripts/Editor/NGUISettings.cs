...
public class NGUISettings
{
    ...
    static public UIButton AddButton(GameObject go)
    {
        UIButton w = go.GetComponent<UIButton>();
       
        if (w == null)
        {
            w = go.AddComponent<UIButton>();
        }

        //w.name = "btn";
        w.tweenTarget = go.GetComponent<UISprite>().gameObject;
